...
export class MusicScheduleEntity extends BaseEntity {
  ...
  @Index()
  @Column({ name: 'schedule_date', comment: '上课日期', type: 'varchar', length: 10 })
  scheduleDate: string;

  @Column({ name: 'start_time', comment: '开始时间', type: 'varchar', length: 5 })
  startTime: string;

  @Column({ name: 'end_time', comment: '结束时间', type: 'varchar', length: 5 })
  endTime: string;
